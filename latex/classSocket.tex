\hypertarget{classSocket}{}\doxysection{Référence de la classe Socket}
...
Assigns the socket to an IP address. On Unix/\+Linux host can be a hostname, on Windows it can only be an IP address. \begin{DoxyReturn}{Renvoie}
...
Assigns the socket to localhost. \begin{DoxyReturn}{Renvoie}
0 on success or a negative value on error, see \mbox{\hyperlink{classSocket_a9f68308228badcdd299cd83e62e36976}{Socket\+::\+Errors}} 
\end{DoxyReturn}
...
Connects the socket to an address. Typically used for connecting TCP/\+IP clients to a \mbox{\hyperlink{classServerSocket}{Server\+Socket}}. On Unix/\+Linux host can be a hostname, on Windows it can only be an IP address. \begin{DoxyReturn}{Renvoie}
...
Receives data from a connected (TCP/\+IP) socket. Reads at most {\itshape len} bytes fand stores them in {\itshape buf}. By default, this function blocks the caller until thre is availbale data. \begin{DoxyReturn}{Renvoie}
...
Send sdata to a connected (TCP/\+IP) socket. Sends the first {\itshape len} bytes in {\itshape buf}. \begin{DoxyReturn}{Renvoie}
